...
public abstract class GeneratedEmailAddress extends GenericItem
{
	...
	**/
	...
	{
		ComposedType relationSecondEnd0 = TypeManager.getInstance().getComposedType("EmailMessage");
		if(relationSecondEnd0.isAssignableFrom(referencedItem.getComposedType()))
		{
			return Utilities.getMarkModifiedOverride(EMAILMESSAGE2TOADDRESSESREL_MARKMODIFIED);
		}
		ComposedType relationSecondEnd1 = TypeManager.getInstance().getComposedType("EmailMessage");
		if(relationSecondEnd1.isAssignableFrom(referencedItem.getComposedType()))
		{
			return Utilities.getMarkModifiedOverride(EMAILMESSAGE2CCADDRESSESREL_MARKMODIFIED);
		}
		ComposedType relationSecondEnd2 = TypeManager.getInstance().getComposedType("EmailMessage");
		if(relationSecondEnd2.isAssignableFrom(referencedItem.getComposedType()))
		{
			return Utilities.getMarkModifiedOverride(EMAILMESSAGE2BCCADDRESSESREL_MARKMODIFIED);
		}
		return true;
	}
