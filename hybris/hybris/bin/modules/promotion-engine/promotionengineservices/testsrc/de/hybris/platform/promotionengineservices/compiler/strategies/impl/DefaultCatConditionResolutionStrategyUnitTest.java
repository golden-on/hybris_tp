...
public class DefaultCatConditionResolutionStrategyUnitTest
{
	...
	public void shouldClearStoredParameterValuesForRuleAndModule()
	{
		...
		when(promotionSourceRuleDao.findAllCatForPromotionSourceRule(rule, BASE_NAME)).thenReturn(catsForRule);
		...
		when(promotionSourceRuleDao.findAllCombinedCatsForRule(rule, BASE_NAME)).thenReturn(combinedCatsForRule);
		...
		when(promotionSourceRuleDao.findAllExcludedProductForPromotionSourceRule(rule, BASE_NAME)).thenReturn(excludedProductsForRule);

		final DroolsKIEModuleModel kieModule = mock(DroolsKIEModuleModel.class);
		when(rulesModuleDao.findByName(MODULE_NAME)).thenReturn(kieModule);

		final DroolsKIEBaseModel kieBase = mock(DroolsKIEBaseModel.class);
		when(kieBase.getName()).thenReturn(BASE_NAME);
		when(droolsKIEBaseFinderStrategy.getKIEBaseForKIEModule(kieModule)).thenReturn(kieBase);

		doNothing().when(modelService).removeAll(anyList());
		//when
		strategy.cleanStoredParameterValues(context);
		//then
		verify(promotionSourceRuleDao, times(1)).findAllCatForPromotionSourceRule(rule, BASE_NAME);
		

		verify(promotionSourceRuleDao, times(1)).findAllCombinedCatsForRule(rule, BASE_NAME);
		verify(modelService, times(1)).removeAll(combinedCatsForRule);

		verify(promotionSourceRuleDao, times(1)).findAllExcludedProductForPromotionSourceRule(rule, BASE_NAME);
		verify(modelService, times(1)).removeAll(excludedProductsForRule);

		verify(promotionSourceRuleDao, times(1)).findAllExcludedCatForPromotionSourceRule(rule, BASE_NAME);
	}
}
