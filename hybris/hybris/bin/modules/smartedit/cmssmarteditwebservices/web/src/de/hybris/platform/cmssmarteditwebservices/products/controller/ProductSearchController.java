...
public class ProductSearchController
{
	...
	public ProductSearchResultWsDTO findProductsByTextOrMask(final String text, final String mask, final PageableWsDTO pageableDto)
	{
		return findProductsByTextOrMask(text, mask, pageableDto, null);
	}

	@RequestMapping(method = RequestMethod.GET, params = { "pageSize" })
	@ResponseBody
	@ApiOperation(value = "Find products by text or mask and language iso code", notes = "Endpoint to retrieve products using a free text search field.")
	@ApiResponses({
	...
	public ProductSearchResultWsDTO findProductsByTextOrMask(
			...
			@ApiParam(value = "The language iso code by which products will be filtered") @RequestParam(required = false) final String langIsoCode)
	{
		...
		final PageableData pageableData = Optional.of(pageableDto) //
				...
				.orElse(null);
		final SearchResult<ProductData> productSearchResult = getCmsSeProductSearchFacade().findProducts(searchText, pageableData, langIsoCode);
