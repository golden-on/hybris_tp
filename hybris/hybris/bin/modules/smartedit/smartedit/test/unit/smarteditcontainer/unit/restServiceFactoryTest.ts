...
import {IExtensibleResourceClass, IRestService, IURIBuilder, Page, Pageable, Payload, TypedMap} from 'smarteditcommons';
...
describe('test RestServiceFactory ', () => {
	...
	class URIBuilder implements IURIBuilder {
		...
		constructor(private auri: string) {
		...
		replaceParams(params: TypedMap<string>) {
			const clone = lo.cloneDeep(this);
			if (params) {
				...
				keys.forEach(function(key) {
					...
					if (params[key]) {
						clone.auri = clone.auri.replace(':' + key, params[key]).replace(re, params[key]);
					}
				});
			}
			return clone;
	...
	beforeEach(() => {
		...
		restServiceFactory = new RestServiceFactory($q, $resource, lo, URIBuilder);
	...
	it('calls $resource and adds getById, get, query, page, save, update and remove custom methods', () => {
		...
		expect(Object.keys(methodMap)).toEqual(['getById', 'get', 'query', 'page', 'update', 'save', '_saveAndGetArray', 'remove']);
	});
