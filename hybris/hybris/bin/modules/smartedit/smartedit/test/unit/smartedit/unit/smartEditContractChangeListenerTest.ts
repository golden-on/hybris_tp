...
describe('smartEditContractChangeListener in polyfill mode', () => {
	...
	beforeEach(() => {
		...
		componentHandlerService = jasmine.createSpyObj('componentHandlerService', ['getClosestSmartEditComponent', 'isSmartEditComponent', 'getFirstSmartEditComponentChildren', 'getParent']);
	...
	beforeEach(() => {
		...
		(directParent as any).getAttribute = (): string => {
			return null;
		};
		...
		(component0 as any).getAttribute = (): string => {
			return null;
		};
		...
		(component1 as any).getAttribute = (): string => {
			return null;
		};
		...
		(component21 as any).getAttribute = (): string => {
			return null;
		};
		...
		(component2 as any).getAttribute = (): string => {
			return null;
		};
		...
		(component3 as any).getAttribute = (): string => {
			return null;
		};
		...
		(invisibleComponent as any).getAttribute = (): string => {
			return null;
		};
		...
		(nonProcessableComponent as any).getAttribute = (): string => {
			return null;
		};
		...
		(detachedComponent as any).getAttribute = (): string => {
			return null;
		};
	...
	describe('DOM mutations', () => {
		...
		it('should not trigger onComponentChangedCallback when component new attributes are the same as old attributes', async () => {
			// GIVEN
			const MOCK_COMPONENT_ATTRIBUTES: any = {
				ID_ATTRIBUTE: 'random_id',
				UUID_ATTRIBUTE: 'random_uuid'
			};
			(component1 as any).getAttribute = (attr: string): string => {
				return MOCK_COMPONENT_ATTRIBUTES[attr];
			};
			(component1 as any).hasAttribute = () => {
				return true;
			};
			// mutation trigger but all oldValue remain the same
			const mutations = [{
				type: 'attributes',
				attributeName: UUID_ATTRIBUTE,
				target: component1,
				oldValue: MOCK_COMPONENT_ATTRIBUTES[UUID_ATTRIBUTE]
			}, {
				type: 'attributes',
				attributeName: ID_ATTRIBUTE,
				target: component1,
				oldValue: MOCK_COMPONENT_ATTRIBUTES[ID_ATTRIBUTE]
			}];
			let componentsInQueue = getComponents();
			expect(componentsInQueue).toContain(component1);

			intersectionObserverMock.observe.calls.reset();
			...
			componentsInQueue = getComponents();
			expect(componentsInQueue).toContain(component1);
			expect(intersectionObserverMock.observe).not.toHaveBeenCalled();
			expect(intersectionObserverMock.unobserve).not.toHaveBeenCalled();
			expect(onComponentChangedCallback).not.toHaveBeenCalled();
		});
	});
});
