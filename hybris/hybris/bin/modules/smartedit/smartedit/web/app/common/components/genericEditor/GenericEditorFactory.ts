...
export function GenericEditorFactory(
	...
	return class implements IGenericEditor {
		...
		init(): angular.IPromise<{}> {
			...
			this._setApi({
				...
				addContentChangeEvent: (event: () => void) => {
					...
					return () => {
						const index = this.onChangeEvents.findIndex((e: any) => {
							return e === event;
						});
				...
				triggerContentChangeEvents: () => {
					this.onChangeEvents.forEach((event: any) => {
						event(copy(this.component));
					});
				},
				 ...
				 * within the editor.
				 * @return {angular.IPromise<ILanguage[]>} a promise resolving to language descriptors. Each descriptor provides the following
				 * language properties: isocode, nativeName, name, active, and required.
				 */
				getLanguages: (): angular.IPromise<ILanguage[]> => {
					return null;
				}
			});
		...
		public _buildFieldComparable(fieldValue: string, field: GenericEditorField): string | boolean {
		...
		public validate(conf: IGenericEditorFactoryOptions): void {
