...
angular.module('renderServiceModule', [
    ...
    .service('renderService', function(
        ...
        this._updateComponentSizeAndPosition = function(componentElem, componentOverlayElem) {
            componentElem = yjQuery(componentElem);
            ...
            if (componentOverlayElem) {
                ...
                var shallowCopy = yjQuery(componentOverlayElem)
        ...
        this._cloneComponent = function(element) {
            if (yjQuery(element).is(":visible")) {
                element = yjQuery(element);
                ...
                var componentDecorator = yjQuery(smartEditWrapper);
        ...
        this._resizeSlots = function() {
            componentHandlerService.getFirstSmartEditComponentChildren('body').each(function(index, slotComponent) {
                var slotComponentID = yjQuery(slotComponent).attr(ID_ATTRIBUTE);
                var slotComponentType = yjQuery(slotComponent).attr(TYPE_ATTRIBUTE);

                var newSlotTop = -1;
                var newSlotBottom = -1;

                yjQuery(slotComponent)
                    .find("." + COMPONENT_CLASS)
                    .filter(function(index, componentInSlot) {
                        componentInSlot = yjQuery(componentInSlot);
                ...
                if (Math.abs(currentSlotHeight - newSlotHeight) > 0.001) {
                    ...
                    yjQuery(slotComponent).data('smartedit-resized-slot', true);
                }
            }.bind(this));
        };

        this.resizeSlots = lodash.debounce(this._resizeSlots.bind(this), 50);
        ...
        this.renderSlots = function(_slotIds) {
            ...
            var slotsRemaining = slotIds.filter(function(id) {
                return !seNamespaceService.renderComponent(id, 'ContentSlot');
            });
                ...
                return experienceService.buildRefreshedPreviewUrl().then(function(storefrontUrl) {
                    return $http({
                        ...
                        slotsRemaining.forEach(function(slotId) {
                            ...
                            var originalSlot = yjQuery(slotSelector);
        ...
        this._destroyComponent = function(_component, _parent, oldAttributes) {
            var component = yjQuery(_component);
            var parent = yjQuery(_parent);
