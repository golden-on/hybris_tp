...
export class PerspectiveService extends IPerspectiveService {
	...
	constructor(
		...
		private PERSPECTIVE_SELECTOR_WIDGET_KEY: string,
		private EVENT_PERSPECTIVE_UPDATED: string,
	...
	register(configuration: IPerspective): angular.IPromise<void> {
		...
		if (!perspective) {
			...
			this.systemEventService.publishAsync(this.EVENT_PERSPECTIVE_UPDATED);
		}
	...
	selectDefault(): angular.IPromise<void> {
		return this.getPerspectives().then((perspectives: IPerspective[]) => {
			return this.$q.when(this.storageService.getValueFromLocalStorage(this.PERSPECTIVE_COOKIE_NAME, true)).then((cookieValue: string) => {
	...
	private _addPerspectiveSelectorWidget(configuration: IPerspective) {
		...
		if (configuration.features.indexOf(this.PERSPECTIVE_SELECTOR_WIDGET_KEY) === -1) {
			configuration.features.unshift(this.PERSPECTIVE_SELECTOR_WIDGET_KEY);
		}
	}
