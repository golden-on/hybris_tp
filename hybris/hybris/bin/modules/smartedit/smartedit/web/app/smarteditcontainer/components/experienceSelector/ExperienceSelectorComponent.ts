...
import {
	...
	IPreviewData, ISeComponent, ISharedDataService, ISite, Payload, SeComponent, SystemEventService
...
export class ExperienceSelectorComponent implements ISeComponent {
	...
	$onInit() {
		this.resetExperienceSelector = () => {
			this.$q.all([
				...
				experienceContent.previewCatalog = `${experience.siteDescriptor.uid}|${experience.catalogDescriptor.catalogId}|${experience.catalogDescriptor.catalogVersion}`;
	...
	preparePayload(experienceContent: IExperienceContent): angular.IPromise<IPreviewData> {
		[this.siteCatalogs.siteId, this.siteCatalogs.catalogId, this.siteCatalogs.catalogVersion] = experienceContent.previewCatalog.split('|');
		...
		return this.catalogService.getProductCatalogsForSite(this.siteCatalogs.siteId).then((productCatalogs: IBaseCatalog[]) => {
			...
			return this.$q.all([
				...
				transformedPayload.siteId = siteDescriptor.uid;
	...
	updateCallback(payload: IExperienceContent, response: IPreviewData) {
