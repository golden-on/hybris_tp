...
import {PageInfoService} from 'smartedit/services';
...
export class ComponentHandlerService {
	 ...
	 */
	getOverlay(): JQuery {
		return this.yjQuery('#' + this.OVERLAY_ID);
	}
	...
	getParentSlotForComponent(component: HTMLElement | JQuery): string {
		const parent: JQuery = this.yjQuery(component).closest('[' + this.TYPE_ATTRIBUTE + '=' + this.CONTENT_SLOT_TYPE + ']');
	...
	getSlotOperationRelatedUuid(component: HTMLElement | JQuery): string {
		const containerId: string = this.yjQuery(component).attr(this.CONTAINER_ID_ATTRIBUTE);
		return containerId && this.yjQuery(component).attr(this.CONTAINER_TYPE_ATTRIBUTE) ? containerId : this.yjQuery(component).attr(this.UUID_ATTRIBUTE);
	}
	...
	getClosestSmartEditComponent(component: HTMLElement | JQuery): JQuery {
		return this.yjQuery(component).closest("." + this.COMPONENT_CLASS);
	}
	...
	isSmartEditComponent(component: HTMLElement | JQuery): boolean {
		return this.yjQuery(component).hasClass(this.COMPONENT_CLASS);
	}
	...
	setId(component: HTMLElement | JQuery, id: string): JQuery {
		return this.yjQuery(component).attr(this.ID_ATTRIBUTE, id);
	}
	...
	getId(component: HTMLElement | JQuery): string {
		return this.yjQuery(component).attr(this.ID_ATTRIBUTE);
	}
	...
	getUuid(component: HTMLElement | JQuery): string {
		return this.yjQuery(component).attr(this.UUID_ATTRIBUTE);
	}
	...
	getCatalogVersionUuid(component: HTMLElement | JQuery): string {
		return this.yjQuery(component).attr(this.CATALOG_VERSION_UUID_ATTRIBUTE);
	}
	...
	setType(component: HTMLElement | JQuery, type: string): JQuery {
		return this.yjQuery(component).attr(this.TYPE_ATTRIBUTE, type);
	}
	...
	getType(component: HTMLElement | JQuery): string {
		return this.yjQuery(component).attr(this.TYPE_ATTRIBUTE);
	}
	...
	getSlotOperationRelatedType(component: HTMLElement | JQuery): string {
		const containerType: string = this.yjQuery(component).attr(this.CONTAINER_TYPE_ATTRIBUTE);
		return containerType && this.yjQuery(component).attr(this.CONTAINER_ID_ATTRIBUTE) ? containerType : this.yjQuery(component).attr(this.TYPE_ATTRIBUTE);
	}
	...
	isExternalComponent(smarteditComponentId: string, smarteditComponentType: string): boolean {
		...
		return componentCatalogVersionUuid !== this.getBodyClassAttributeByRegEx(PageInfoService.PATTERN_SMARTEDIT_CATALOG_VERSION_UUID);
	}
	...
	getFirstSmartEditComponentChildren(htmlElement: HTMLElement | JQuery): JQuery[] {
		...
		const collection = Array.from(root.getElementsByClassName(this.COMPONENT_CLASS)).filter(
			(element: HTMLElement) => {
                 ...
                 */
				while (current !== root) {
					if (current.classList.contains(this.COMPONENT_CLASS)) {
						return false;
					}
	...
	getComponentCloneInOverlay(component: JQuery): JQuery {
		...
		return this.yjQuery("." + this.OVERLAY_COMPONENT_CLASS + "[" + this.ELEMENT_UUID_ATTRIBUTE + "='" + elementUuid + "']");
	}
