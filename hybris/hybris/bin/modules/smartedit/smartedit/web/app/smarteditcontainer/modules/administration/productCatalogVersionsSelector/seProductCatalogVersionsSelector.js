...
angular.module('seProductCatalogVersionsSelectorModule', ['smarteditServicesModule', 'modalServiceModule', 'multiProductCatalogVersionSelectorModule', 'yLoDashModule', 'l10nModule', 'seDropdownModule'])
    ...
    .controller("seProductCatalogVersionsSelectorController", function($q, $log, $translate, lodash, l10nFilter, catalogService, modalService, systemEventService, PRODUCT_CATALOG_SINGLE_TEMPLATE, PRODUCT_CATALOG_MULTIPLE_TEMPLATE, MODAL_BUTTON_ACTIONS, MODAL_BUTTON_STYLES, MULTI_PRODUCT_CATALOGS_UPDATED, LINKED_DROPDOWN, windowUtils) {
        ...
        this._resetSelector = function(eventId, handle) {
            if (handle.qualifier === 'previewCatalog') {
                if (handle.optionObject && this.initialPreview !== handle.optionObject.id) {

                    this.initialPreview = handle.optionObject.id;

                    var siteId = handle.optionObject.id.split('|')[0];
        ...
        this._setContent = function() {
            ...
            catalogService.getProductCatalogsForSite(this.initialPreview.split('|')[0]).then(function(productCatalogs) {
                this.productCatalogs = productCatalogs;
