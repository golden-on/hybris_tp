	...
	*/
	...
	_rawProcessQueue(): void {
		...
		this.systemEventService.publish(
			this.CONTRACT_CHANGE_LISTENER_PROCESS_EVENTS.PROCESS_COMPONENTS,
			...
			function(observedQueue: ComponentEntry[], response: HTMLElement[]) {
				...
				observedQueue.forEach((obj: ComponentEntry) => {
					...
					const processStatus = obj.component.dataset[this.SMARTEDIT_COMPONENT_PROCESS_STATUS];
	...
	_removeComponents(componentsObj: RemoveComponentObject[], forceRemoval: boolean = false): void {
		componentsObj.filter((queueObj: RemoveComponentObject) => {
			return !this.isInDOM(queueObj.component) || forceRemoval;
		...
		if (this._componentsRemovedCallback) {
			const removedComponents = componentsObj.map((obj: RemoveComponentObject) => {
				return this.lodash.pick(obj, ['component', 'parent', 'oldAttributes']);
			});
	...
	private aggregateAddedOrRemovedNodesAndTheirParents(mutations: MutationRecord[], type: MUTATION_CHILD_TYPES): AggregatedNode[] {
					...
					.flatten<HTMLElement>(
						Array.from(mutation[type])
							...
							.map((child: HTMLElement) => {
								return this.componentHandlerService.isSmartEditComponent(child) ? child : Array.from(
									this.componentHandlerService.getFirstSmartEditComponentChildren(child) as any
								);
							})
					)
		...
		return this.lodash.uniqWith(this.lodash.flatten(entries), (entry1, entry2) => {
	...
	/**
	 * Verifies whether the entry is a smartedit compliant element.
	 */
	private isSmarteditNode(node: HTMLElement): boolean {
		return this.componentHandlerService.isSmartEditComponent(node) &&
	 ...
	 */
	private wasSmarteditNode(node: HTMLElement): boolean {
		return (
			this.componentsQueue.has(node) &&
			this.smartEditAttributeNames.every((attributeName) => {
				return !node.hasAttribute(attributeName);
			})
		);
	 ...
	 */
	private hasSmarteditOldAttributeChanged(entry: TargetedNode): boolean {
		return this.smartEditAttributeNames.some((attributeName) => {
			return this.smartEditAttributeNames.indexOf(attributeName) > -1; return entry.oldAttributes.hasOwnProperty(attributeName);
		});
	}

	/**
	 * Check whether the entry has one or more smartedit properties value changed.
	 */
	private hasSmarteditNewAttributeChanged(entry: TargetedNode) {
		return this.smartEditAttributeNames.some((attributeName: string) => {
			return entry.oldAttributes[attributeName] &&
				entry.node.hasAttribute(attributeName) &&
				entry.node.getAttribute(attributeName) !== entry.oldAttributes[attributeName];
		});
	}
	...
	private mutationsHasPageChange(mutations: MutationRecord[]): MutationRecord {
		return mutations.find((mutation: MutationRecord) => {
			...
			return mutation.type === MUTATION_TYPES.ATTRIBUTES.NAME && element.tagName === "BODY" && mutation.attributeName === "class";
		});
	}
	...
	private mutationObserverCallback(mutations: MutationRecord[]): void {
		...
		if (this._componentsAddedCallback) {
			this.aggregateAddedOrRemovedNodesAndTheirParents(mutations, MUTATION_TYPES.CHILD_LIST.ADD_OPERATION).forEach((childAndParent: AggregatedNode) => {
				this.applyToSelfAndAllChildren(childAndParent.node, this.intersectionObserver.observe.bind(this.intersectionObserver));
			});
		}

		this.aggregateAddedOrRemovedNodesAndTheirParents(mutations, MUTATION_TYPES.CHILD_LIST.REMOVE_OPERATION).forEach((childAndParent: AggregatedNode) => {
		...
		if (this._onComponentChangedCallback) {
			this.aggregateMutationsOnChangedAttributes(mutations).forEach((entry: TargetedNode) => {
				if (this.hasSmarteditOldAttributeChanged(entry)) {
					if (this.isSmarteditNode(entry.node)) {
						...
						} else if (this.hasSmarteditNewAttributeChanged(entry)) {
					...
					} else if (this.wasSmarteditNode(entry.node)) {
						this.applyToSelfAndAllChildren(entry.node, this.intersectionObserver.unobserve.bind(this.intersectionObserver));
						const parents = this.componentHandlerService.getClosestSmartEditComponent(entry.node as HTMLElement);
						this._removeComponents([{
							isIntersecting: false,
