...
export class RestService<T> implements IRestService<T> {
	...
	_saveAndGetArray(payload: Payload): angular.IPromise<T[]> {
		return this.delegateRestService.delegateForArray("_saveAndGetArray", payload, this.uri, this.identifier, this.metadataActivated);
	}
