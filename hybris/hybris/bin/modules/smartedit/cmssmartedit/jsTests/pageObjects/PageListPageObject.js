...
module.exports = function() {
    ...
    pageList.selectors = {
        getColumnHeaderForKeySelector: function(key) {
            return by.css('.se-paged-list-table thead tr:first-child .se-paged-list__header-' + key);
        },
    ...
    pageList.elements = {
        getTotalPageCount: function() {
            browser.waitForPresence(element(by.css('.se-paged-list__page-count-wrapper')), 'cannot find page list count item');
        ...
        getPaginationCount: function() {
            return element.all(
                by.css('.fd-pagination > ul > li')).count().then(
                function(count) {
        ...
        getDropdownEditButton: function() {
            return element(
                by.cssContainingText('y-drop-down-menu .open li', 'Edit'));
        },

        getDropdownMoveToTrashButton: function() {
            return element(
                by.cssContainingText('y-drop-down-menu .open li', 'Move to Trash'));
        },
        ...
        getSyncedPageSyncIcon: function() {
            return element(by.css(
                '.se-paged-list__table-body > tr:nth-child(2) page-list-sync-icon .IN_SYNC'));
        },

        getFirstPageSyncIcon: function() {
            return element(by.css(
                '.se-paged-list__table-body > tr:first-child page-list-sync-icon .IN_SYNC'));
        },
        ...
        getFailureAlert: function() {
            return element(by.css('system-alerts .fd-alert--error'));
        },
    ...
    pageList.actions = {
        ...
        navigateToIndex: function(index) {
            return browser.executeScript('window.scrollTo(0,document.body.scrollHeight);').then(function() {
                return browser.click(by.cssContainingText('.fd-pagination  > ul > li a', index.toString())).then(function() {
        ...
        clickSyncPageStatusButton: function() {
            browser.waitForPresence(pageList.elements.getDropdownSyncPageStatusButton(), "Expected Sync Page Status option to be available in the dropdown.");
            browser.click(pageList.elements.getDropdownSyncPageStatusButton(), "Could not click on the Sync Page Status option in the dropdown.");
            browser.waitForPresence(pageList.elements.getModalDialog(), "Expected the presence of a modal window.");
        },
        // Restrictions Viewer
    ...
    pageList.assertions = {
        ...
        assertOnPageSyncIconStatusByPageIndex: function(pageIndex, expectedStatus) {
            browser.waitForPresence(element(by.css(".se-paged-list__table-body .se-paged-list-item")));
            var actualStatus = null;
            browser.waitUntil(function() {
                return element(by.css('.se-paged-list__table-body > tr:nth-child(' + pageIndex + ') page-list-sync-icon span')).getAttribute("data-sync-status").then(function(_actualStatus) {
        ...
        totalPageCount: function(count) {
            browser.waitForPresence(element(by.css('.se-paged-list__page-count-wrapper')), 'cannot find page list count item');
            browser.waitForSelectorToContainText(pageList.selectors.getTotalPageCountSelector(), count);
        },
        ...
        permanentlyDeleteButtonCanBeClicked: function(pageName) {
            pageList.actions.openPageDropdownByPageName(pageName);
            browser.waitForAbsence(pageList.elements.getDropdownPermanentlyDeleteButton().element(by.css('a.se-dropdown-item__disabled')));
        },
