...
(function() {
    ...
    dragAndDropPageObject.structure = {
        slots: {
            ...
            TYPES_UNKNOWN_SLOT: 'deepLinksSlot'
        },
        components: {
    ...
    dragAndDropPageObject.elements = {
        ...
        getParagraphComponentInComponentMenu: function() {
            return element(by.css(".smartEditComponent[data-smartedit-component-type='CMSParagraphComponent']"));
        },
        getAbstractCMSComponentInComponentMenu: function() {
            return element(by.css("y-infinite-scrolling .smartEditComponent[data-smartedit-component-type='AbstractCMSComponent'][data-smartedit-component-id='Component10']"));
        },
    ...
    dragAndDropPageObject.actions = {
        ...
        isSlotMayBeEnabled: function(slotId) {
            var slot = storefront.elements.getComponentInOverlayById(slotId);
            return hasClass(slot, 'over-slot-maybeenabled');
        },
    ...
    function _dropComponent(componentId, slotId) {
        var slot = storefront.elements.getComponentInOverlayById(slotId);
        return browser
            .executeScript(simulateDragAndDropOperation, slot.getWebElement(), EVENT_TYPES.DROP)
            .then(function() {
                return browser.waitUntilNoModal().then(function() {
                    return browser.executeScript(
                        simulateDragAndDropOperation,
                        storefront.elements.getComponentInOverlayById(componentId).getWebElement(),
    ...
    function _isComponentInPosition(slotId, componentId, position, expectedElementsInSlotCount) {
            ...
            return storefront.elements.getComponentsBySlotId(slotId, expectedElementsInSlotCount)
                .then(function(components) {
                    if (components.length > position) {
                        return components[position].getAttribute('data-smartedit-component-id').then(function(attr) {
        ...
        }, "at position " + position + ", expected: " + componentId);
