...
describe('Page Service ->', () => {
	...
	const pagesFallbacksRestService = jasmine.createSpyObj<any>(
		'pagesFallbackRestService',
		['getFallbacksForPageId', 'getFallbacksForPageIdAndContext']
	);
	const pagesVariationsRestService = jasmine.createSpyObj<any>(
		'pagesVariationsRestService',
		['getVariationsForPrimaryPageId']
	);
	const cmsitemsRestService = jasmine.createSpyObj<any>('cmsitemsRestService', [
		'get',
		'getById',
		'getByIdAndVersion',
		'update',
	]);
	const experienceService = jasmine.createSpyObj<IExperienceService>(
		'experienceService',
		['getCurrentExperience']
	);
	const pageInfoService = jasmine.createSpyObj<IPageInfoService>(
		'pageInfoService',
		['getPageUUID']
	);
	const urlService = jasmine.createSpyObj<IUrlService>('urlService', [
		'buildUriContext',
	]);
	...
	describe('initialization', () => {
		it('WHEN initialized THEN it must be GatewayProxied', () => {
			expect(
				annotationService.getClassAnnotation(PageService, GatewayProxied)
			).toEqual([]);
		});
	});
	...
	describe('isPagePrimary', () => {
		...
		it('GIVEN no fallbacks exist for the current page WHEN isPagePrimary is called THEN it should return true', () => {
			...
			expect(
				pagesFallbacksRestService.getFallbacksForPageId
		...
		it('GIVEN no fallbacks exist for the current page WHEN isPagePrimaryWithContext is called THEN it should return true', () => {
			...
			// WHEN
			const promise = pageService.isPagePrimaryWithContext(
				PAGE_UID,
				URI_CONTEXT
			);
			...
			expect(promise).toBeResolvedWithData(true);
			expect(
				pagesFallbacksRestService.getFallbacksForPageIdAndContext
			).toHaveBeenCalledWith(PAGE_UID, URI_CONTEXT);
		});
	...
	describe('updatePageById', () => {
		it('WHEN updatePageById is called THEN it updates the page with the provided information', () => {
			...
			expect(pagesRestService.update).toHaveBeenCalledWith(
				PAGE_UID,
				newPageInfo
			);
		});
	...
	describe('primaryPageForPageTypeExists', () => {
		it('GIVEN primary page for page type exists THEN it should return true', (done) => {
			// GIVEN
			...
			// WHEN
			pageService.primaryPageForPageTypeExists(PAGE_TYPE).then((actual) => {
				// THEN
		...
		it('GIVEN primary page for page type does not exist THEN it should return false', (done) => {
			// GIVEN
			cmsitemsRestService.get.and.returnValue(
				$q.resolve({
					response: [],
	...
	describe('getPaginatedPrimaryPagesForPageType', () => {
		it(
			...
			() => {
				pageService.getPaginatedPrimaryPagesForPageType(PAGE_TYPE, URI_CONTEXT);
		...
		it(
			...
			() => {
				// currentPage
				pageService.getPaginatedPrimaryPagesForPageType(
					PAGE_TYPE,
					URI_CONTEXT,
					{
						search: 'somelabel',
		...
		it('WHEN is called THEN it returns the paginated list of primary pages for the given page type', () => {
			cmsitemsRestService.get.and.returnValue(
				$q.resolve({
					response: [page],
				})
			...
			// WHEN
			pageService
				.getPaginatedPrimaryPagesForPageType(PAGE_TYPE, URI_CONTEXT)
				.then((value) => {
					// THEN
