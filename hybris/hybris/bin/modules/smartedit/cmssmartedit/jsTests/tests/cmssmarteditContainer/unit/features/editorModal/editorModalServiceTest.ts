...
describe('editorModalService', () => {
	...
	let contextAwareEditableItemService: jasmine.SpyObj<
		ContextAwareEditableItemService
	>;
	let cmsitemsRestService: jasmine.SpyObj<any>;
	let $translate: jasmine.SpyObj<any>;
	let generateIdentifier: jasmine.Spy;
	let genericEditorStackService: jasmine.SpyObj<any>;
	...
	const COMPONENT_SHARED_IN_WORKFLOW_CONTEXT_MESSAGE = 'se.cms.component.workflow.shared.component';
	beforeEach(() => {
		COMPONENT_SERVICE_SLOTS_FOR_COMPONENT = ['some-slot-id'];
		renderServiceMock = jasmine.createSpyObj('renderService', ['renderSlots']);
		genericEditorModalServiceMock = jasmine.createSpyObj(
			'genericEditorModalServiceMock',
			['open']
		);
		contextAwareEditableItemService = jasmine.createSpyObj(
			'contextAwareEditableItemService',
			['isItemEditable']
		);
		...
		$translate = jasmine.createSpyObj<angular.translate.ITranslateService>(
			'$translate',
			['instant']
		);
		...
		componentService.getSlotsForComponent.and.returnValue(
			promiseHelper.buildPromise<string[]>('openGenericModalPromise', PromiseType.RESOLVES, COMPONENT_SERVICE_SLOTS_FOR_COMPONENT)
		);
		cmsitemsRestService.getById.and.returnValue($q.resolve({
			uid: COMPONENT_UID
		}));
		...
		generateIdentifier = jasmine.createSpy();
		genericEditorStackService = jasmine.createSpyObj('genericEditorStackService', ['getEditorsStack']);
		genericEditorStackService.getEditorsStack.and.returnValue([
			{component: {uuid: 'some-slot-id'}}
		]);
	...
	it('open will delegate to genericEditorModalService.open and invoke a rerendering upon closing', () => {
		// GIVEN
		generateIdentifier.and.returnValue('identifier');
		...
		expect(genericEditorModalServiceMock.open.calls.argsFor(0)[0]).toEqual(genericEditorModalServiceEditComponentMock);
		...
		callback();
		expect(renderServiceMock.renderSlots).toHaveBeenCalledWith(COMPONENT_SERVICE_SLOTS_FOR_COMPONENT);
	});
	it('GIVEN creating a new component THEN renderService.renderComponent is not called', () => {
		...
		const genericEditorModalServiceCreateComponentMock: IGenericEditorModalServiceComponent = {
			...
			componentType:
				editorModalServiceCreateComponentMock.smarteditComponentType,
			title:
				'type.' +
			...
			content: {
				...
				catalogVersion:
					editorModalServiceCreateComponentMock.catalogVersionUuid,
				visible: true,
				position: undefined,
			},
			initialDirty: false,
			editorStackId: undefined,
		};
		// WHEN
		editorModalService.open(editorModalServiceCreateComponentMock);
		// THEN
		expect(renderServiceMock.renderSlots).not.toHaveBeenCalled();
	...
	it("should mark component read only if it is not editable in current context", () => {
		...
		editorModalService.open(editorModalServiceEditComponentMock);
		// THEN
		const genericEditorParams = genericEditorModalServiceMock.open.calls.argsFor(
			0
		)[0];
	...
	it("should display only one message if component is editable and is shared in current context at the same time", () => {
		...
		$translate.instant.withArgs(COMPONENT_SHARED_IN_WORKFLOW_CONTEXT_MESSAGE).and.returnValue(COMPONENT_SHARED_IN_WORKFLOW_CONTEXT_MESSAGE);
		$translate.instant.withArgs(COMPONENT_SHARED_IN_PAGE_CONTEXT_MESSAGE).and.returnValue(COMPONENT_SHARED_IN_PAGE_CONTEXT_MESSAGE);
		// WHEN
		editorModalService.open(editorModalServiceEditComponentMock);
		// THEN
		const genericEditorParams = genericEditorModalServiceMock.open.calls.argsFor(
