...
describe('Drag and Drop -', function() {
    ...
    describe('within frame', function() {
        ...
        it('WHEN I grab a component from one slot AND hover over another slot in which its type is may be allowed THEN I should see the slot highlighted to indicate that drop is allowed', function() {
            // Arrange
            dragAndDrop.actions.moveToStoreFront();

            // Act
            dragAndDrop.actions.grabComponent(slots.TOP_HEADER_SLOT, components.COMPONENT1);
            storefront.actions.moveToComponent(storefront.constants.TYPES_UNKNOWN_SLOT_ID);
            var typesUnknownSlot = storefront.elements.getComponentInOverlayById(
                slots.TYPES_UNKNOWN_SLOT
            );
            dragAndDrop.actions.hoverOverElement(typesUnknownSlot);

            // Assert
            expect(dragAndDrop.actions.isSlotEnabled(slots.TYPES_UNKNOWN_SLOT)).toBe(
                true,
                'Expected slot to display that it allows the component'
            );
            expect(dragAndDrop.actions.isSlotMayBeEnabled(slots.TYPES_UNKNOWN_SLOT)).toBe(
                true,
                'Expected slot to display that may be it allows the component'
            );
            expect(dragAndDrop.actions.isSlotDisabled(slots.TYPES_UNKNOWN_SLOT)).toBe(
                false,
                'Expected slot not to display that the component is forbidden'
            );
        });
    ...
    describe('across frames', function() {
        ...
        it('WHEN I grab a component from the component menu AND drop it in a slot with hidden components THEN the component is moved to the new position',
            function() {
                var TOP_HEADER_TOTAL_COMPONENTS = 3;
                perspective.selectBasicPerspective();

                dragAndDrop.actions.grabCustomizedComponentInComponentMenu();
                dragAndDrop.actions.addFromComponentMenuToSlotInPosition(slots.TOP_HEADER_SLOT, 3, TOP_HEADER_TOTAL_COMPONENTS);

                // Assert
                expect(dragAndDrop.actions.isComponentInPosition(slots.TOP_HEADER_SLOT, components.COMPONENT10, 2, TOP_HEADER_TOTAL_COMPONENTS + 1)).toBe(true, 'Expected new component to move to the 2nd position of the top header slot');
            });
    ...
    describe('scrolling', function() {
        ...
        it('WHEN I grab a component AND I position the mouse in the top hint THEN the page scrolls up',
            function() {
                dragAndDrop.actions.grabComponentInComponentMenu();
                dragAndDrop.actions.moveToStoreFront();
