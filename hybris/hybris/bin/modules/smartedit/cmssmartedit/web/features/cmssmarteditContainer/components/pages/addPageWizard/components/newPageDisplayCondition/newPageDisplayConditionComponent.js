...
angular.module('newPageDisplayConditionModule', ['pageServiceModule', 'pageDisplayConditionsServiceModule'])
    ...
    .controller('newPageDisplayConditionController', function(
        ...
        this.$onChanges = function $onChanges(changes) {
            if ((changes.pageTypeCode && this.pageTypeCode !== this.previousPageTypeCode) ||
                ...
                if (!this.targetCatalogVersion || this._isUriContextEqualToCatalogVersion(this.uriContext, this.targetCatalogVersion)) {
                    // For some reason at the first time modal is opened $onChanges is called
                    // immediately twice and promise response in _getSelectedPrimaryPageAndDisplayCondition
                    // has empty response from API (at least in tests)
                    // causing that default page is not selected / bound to ui dropdown
                    setTimeout(function() {
                        this._getSelectedPrimaryPageAndDisplayCondition();
                    }.bind(this));
                } else {
        ...
        this._getSelectedPrimaryPageAndDisplayCondition = function _getSelectedPrimaryPageAndDisplayCondition() {
            if (this.pageTypeCode) {
                pageService
                    .getPaginatedPrimaryPagesForPageType(this.pageTypeCode, this.uriContext, {
                        search: this.initialPrimaryPageSelected,
                        pageSize: 1,
                        currentPage: 0
                    })
                    .then(
                        function(page) {
                            var primaryPage = page.response[0];
                            this._setPrimarySelected(primaryPage);
                        }.bind(this),
                        function(error) {
                            $log.error(error);
                        }.bind(this)
        ...
        this._setPrimarySelected = function _setPrimarySelected(page) {
            this.primarySelected = page;
            this.primarySelectedModel = page.uid;
        }.bind(this);

        this._primarySelectedModelOnChange = function _primarySelectedModelOnChange(uid) {
            pageService.getPageById(uid).then(
                function(page) {
                    this._setPrimarySelected(page);
                    this.dataChanged();
                }.bind(this)
            );
        }.bind(this);

        this._primaryPageChoicesFetchStrategy = {
            ...
            fetchPage: function(search, pageSize, currentPage) {
                return displayConditionsFacade.getPrimaryPagesForPageType(
                    this.pageTypeCode,
                    this.uriContext, {
                        search: search,
                        pageSize: pageSize,
     ...
     * @param {< String =} targetCatalogVersion An optional string to provide if the target catalog version is selected
     */
