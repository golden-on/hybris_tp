...
import {GatewayProxied, Payload, SeInjectable} from "smarteditcommons";
...
export class EditorModalService extends IEditorModalService {
	constructor(
		...
		private genericEditorStackService: any,
	) {
		super();
	}
	...
	openAndRerenderSlot(componentType: string, componentUuid: string, targetedQualifier: string, saveCallback?: (item: any) => void, editorStackId?: string): angular.IPromise<void> {
		...
		return this._preloadContent(componentUuid, componentAttributes).then((attributes) => {
			const componentData = this._createComponentData(attributes, {
				targetedQualifier
			}, editorStackId);
	...
	open(componentAttributes: IComponent, targetSlotId?: string, position?: number, targetedQualifier?: string, saveCallback?: (item: any) => void, editorStackId?: string): angular.IPromise<void> {
		return this._preloadContent(componentAttributes.smarteditComponentUuid, componentAttributes).then((attributes) => {
			const componentData = this._createComponentData(attributes, {
				slotId: targetSlotId,
				position,
				targetedQualifier
			}, editorStackId || this.generateIdentifier()); // <-- THE CHANGE
	 ...
	 */
	private _preloadContent(uuid: string, componentAttributes: IComponent): angular.IPromise<IComponent> {
	 ...
	 */
	private _openGenericEditor(componentData: IGenericEditorModalServiceComponent, saveCallback?: (item: any) => void): angular.IPromise<void> {
	 ...
	 */
	private _markComponentAsShared(componentContent: Payload, modelServiceParameters: IGenericEditorModalServiceComponent): angular.IPromise<void> {
		if (componentContent && componentContent.uid) {
			return this.contextAwareEditableItemService.isItemEditable(componentContent.uid as string).then((componentIsEditable: boolean) => {
				modelServiceParameters.readOnlyMode = !componentIsEditable;
				if (modelServiceParameters.readOnlyMode) {
					this._addComponentInfoMessage('se.cms.component.workflow.shared.component', modelServiceParameters);
				} else {
					if (this._componentIsShared(componentContent)) {
						this._addComponentInfoMessage('se.cms.component.shared.component', modelServiceParameters);
					}
				}
	 ...
	 */
	private _addComponentInfoMessage(message: string, modelServiceParameters: IGenericEditorModalServiceComponent): void {
	...
	private _prepareContentForCreate(
		...
		preparedContent.position = !this.isBlank(preparedContent.position) ? preparedContent.position : position;
		...
		preparedContent.visible = !this.isBlank(preparedContent.visible) ? preparedContent.visible : true;
		return preparedContent;
	}

	private _createComponentData(componentAttributes: IComponent, params: IComponentAttributes, editorStackId?: string): IGenericEditorModalServiceComponent {
		let type;
		try {
			type = componentAttributes.smarteditComponentType.toLowerCase();
		} catch (e) {
			throw new Error(`editorModalService._createComponentData - invalid component type in componentAttributes. ${e}`);
		}
