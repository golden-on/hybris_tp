...
public class GenericComponentTypeStructureServiceTest
{
	...
	private static final java.lang.String INVALID_TYPE_CODE_REGISTRY = "INVALID_TYPE_CODE_REGISTRY";
	...
	@Mock
	private ComponentTypeStructure typeStructureRegistryInvalid;
	@Mock
	...
	public void setup() throws Exception
	{
		...
		when(typeService.getComposedTypeForCode(INVALID_TYPE_CODE_REGISTRY))
				.thenThrow(new UnknownIdentifierException("Type might be defined in another extension that was not installed"));
		...
		when(componentTypeStructureRegistry.getComponentTypeStructures())
				.thenReturn(Arrays.asList(typeStructureRegistry1, typeStructureRegistry2, typeStructureRegistryInvalid));
		...
		when(componentTypeStructureRegistry.getComponentTypeStructure(INVALID_TYPE_CODE_REGISTRY)).thenReturn(typeStructureRegistryInvalid);
		...
		when(typeStructureRegistryInvalid.getTypecode()).thenReturn(INVALID_TYPE_CODE_REGISTRY);
	...
	public void testGetStructureTypeForCMSItem()
	{
		...
		verify(componentTypeStructureRegistry, times(3)).getComponentTypeStructure(CMSItemModel._TYPECODE);
		verify(componentTypeStructureRegistry, times(3)).getComponentTypeStructure(TYPE_CODE_1);
		verify(componentTypeStructureRegistry, times(3)).getComponentTypeStructure(TYPE_CODE_2);
		verify(componentTypeStructureRegistry).getComponentTypeStructure(TYPE_CODE_REGISTRY);
		
	}

	@Test
	public void testNotGetStructureTypeRegisteredForInvalidType()
	{
		// when the registry contains configuration for an item type that is not valid,
		// that configuration should not be added to the internal ComponentTypeStructureMap
		final ComponentTypeStructure componentTypeStructure = service.getComponentTypeStructure(INVALID_TYPE_CODE_REGISTRY);

		assertThat(componentTypeStructure, nullValue());
		verify(typeService).getComposedTypeForCode(INVALID_TYPE_CODE_REGISTRY);
	}
