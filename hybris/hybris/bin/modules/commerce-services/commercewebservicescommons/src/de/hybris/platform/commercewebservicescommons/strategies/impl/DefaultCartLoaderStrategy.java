...
public class DefaultCartLoaderStrategy implements CartLoaderStrategy
{
	...
	public void loadCart(final String cartId, final boolean refresh)
	{
		...
		if (!CustomerModel.class.isAssignableFrom(currentUser.getClass()))
		{
	...
	protected void loadUserCart(final String cartID, final boolean refresh)
	{
		...
		if (requestedCartID.equals(CURRENT_CART))
		{
			...
			final CartModel cart = commerceCartService
					.getCartForGuidAndSiteAndUser(null, baseSiteService.getCurrentBaseSite(), userService.getCurrentUser());
			if (cart == null)
			{
	...
	/**
	 * Restores a cart. The cart is recalculated if the refresh parameter is true or the cart currency is different than the currency currently set.
	 * It uses {@link CommerceCartService#restoreCart(CommerceCartParameter)} method, which includes logic to verify cart expiration, rebuild cart, delete cart payment transactions,
	 * cart recalculation and current cart setting.
	 *
	 * @param cart
	 * 		user cart
	 ...
	 * 		requested cart id
	 * @param refresh
	 * 		forces recalculation of the cart
	 * @throws CartException
	 * 		thrown in case cart restoration error
	 */
	protected void restoreCart(final CartModel cart, final String requestedCartId, final boolean refresh)
	{
		try
		{
			...
			final boolean currencyTheSame = cart.getCurrency().equals(getCommerceCommonI18NService().getCurrentCurrency());
			parameter.setIgnoreRecalculation(!refresh && currencyTheSame);
