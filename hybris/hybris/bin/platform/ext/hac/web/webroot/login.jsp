...
<form action="<c:url value="/j_spring_security_check"/>" method="POST">
	<div id="logincontrols" class="logincontrols">
		...
		<ul>
			...
			<spring:eval expression="@hybrisProperties.getProperty('hac.login.singlesignon.redirect')" var="hac_sso_url"/>
			<c:if test="${not empty hac_sso_url}">
				<li class="singlesignon_login_cell">
					<a href="${hac_sso_url}">Login with Single Sign On</a>
				</li>
			</c:if>
		</ul>
	</div>
