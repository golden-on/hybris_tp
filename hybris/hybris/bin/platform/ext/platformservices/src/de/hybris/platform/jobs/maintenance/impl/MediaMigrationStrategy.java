...
public class MediaMigrationStrategy implements MaintenanceCleanupStrategy<MediaModel, MediaFolderStructureMigrationCronJobModel>
{
	...
	public void relocateMediaData(final MediaModel media)
	{
		...
		if (currentFolder != null && mediaService.hasData(media))
		{
			...
			try
			{
				mediaData = executeWithDisabledIntegrityVerification(() -> MediaManager.getInstance().getMediaAsStream(currentFolder.getQualifier(), oldLocation));
	...
	static <T> T executeWithDisabledIntegrityVerification(final Supplier<T> supplier)
	{
		final JaloSession session = JaloSession.getCurrentSession();
		final Boolean before = session.getSessionContext().getAttribute(FOLDER_INTEGRITY_VERIFICATION_ENABLED_FLAG);
		try
		{
			session.getSessionContext().setAttribute(FOLDER_INTEGRITY_VERIFICATION_ENABLED_FLAG, false);
			return supplier.get();
		}
		finally
		{
			if (before == null)
			{
				session.getSessionContext().removeAttribute(FOLDER_INTEGRITY_VERIFICATION_ENABLED_FLAG);
			}
			else
			{
				session.getSessionContext().setAttribute(FOLDER_INTEGRITY_VERIFICATION_ENABLED_FLAG, before);
			}
		}
	}
}
