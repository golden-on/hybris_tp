...
public class SessionTest extends HybrisJUnit4Test
{
	...
	private Currency queryCurrency(final String iso)
	{
		return (Currency) FlexibleSearch.getInstance()
		                                .search("SELECT {PK} FROM {Currency} WHERE {isoCode}='" + iso + "'", Currency.class)
		                                .getResult()
		                                .get(0);
	}
	...
	public void testCleanExpiredSessionWithDeletedUser() throws ConsistencyCheckException, JaloSecurityException
	{
		...
		user.getImplementation().remove(jaloSession.getSessionContext());
	...
	public void testJaloSessionPerformLogin() throws ConsistencyCheckException
	{
		...
		{
			if (loginDisabledBefore != null)
			{
				anon.setLoginDisabled(loginDisabledBefore.booleanValue());
			}
	...
	{
		final List<String> col1 = Lists.newArrayList("test");
		final Set<String> col2 = Sets.newHashSet("test");
		final Queue<String> col3 = Queues.newArrayDeque(Collections.singleton("test"));

		final SessionContext sessionContext = jaloSession.getSessionContext();
		sessionContext.setAttribute("col1", col1);
		sessionContext.setAttribute("col2", col2);
		sessionContext.setAttribute("col3", col3);

		final Collection<String> resultCol1 = sessionContext.getAttribute("col1");
		final Collection<String> resultCol2 = sessionContext.getAttribute("col2");
		final Collection<String> resultCol3 = sessionContext.getAttribute("col3");
		assertThat(resultCol1).isInstanceOf(List.class).containsExactly("test");
		assertThat(resultCol2).isInstanceOf(Set.class).containsExactly("test");
		assertThat(resultCol3).isInstanceOf(Queue.class).containsExactly("test");
	}
	...
	private static final class TestPF extends AbstractPriceFactory
	{
		...
		@Override
		public List getProductPriceInformations(final SessionContext ctx, final Product product, final Date date,
		                                        final boolean net)
